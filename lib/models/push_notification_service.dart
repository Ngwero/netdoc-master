...
class PushNotificationService {
  Future<void> setupInteractedMessage() async {
    ...
    FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) {
      // Handle opening the app from a notification (if needed).
  ...
  void initiateCallScreen()async{
    ...
    prefs.setString(kCallKit, _currentUuid);
  ...
  Future<void> registerNotificationListeners() async {
    ...
    flutterLocalNotificationsPlugin.initialize(initSetttings, onSelectNotification: (message) async {
      // Handle notification click when the app is in the foreground.
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage? message) {
      ...
      // Check if the message contains custom data for an incoming call.
      print("YESSSSSSSUUUUUUU the type: ${message.data['type']}, title: ${message.data['title']}, type: ${message.data['title']}");
      if (message.data['type'] == 'incoming_call') {
        String title = message.data['title'] ?? 'Incoming Call';
        String body = message.data['body'] ?? 'You have an incoming call';

        // Show a custom notification for the incoming call.
        flutterLocalNotificationsPlugin.show(
          ...
          NotificationDetails(
            android: AndroidNotificationDetails(
              channel.id,
              channel.name,
              icon: android?.smallIcon ?? '@mipmap/ic_launcher', // Replace with your custom icon.
              playSound: true,
            ),
          ),
        );

        // Trigger the call screen when an incoming call notification is received.
        initiateCallScreen();
      }
    });
  }
