...
class ChatMessaging extends StatefulWidget {
  static String id = 'messaging_page_id';
...
class _ChatMessagingState extends State<ChatMessaging> {
  ...
  Future<void> uploadData() async {
    ...
    return chat.doc(serviceId)
        .set({
      'viewed': false,
      'doctorId': Provider.of<DoctorProvider>(context, listen: false).doctorId,
      'date':  DateTime.now(),
      'message': message,
      'documentId': documentId,
      // 'receiver': 'admin@mynetdok,com',
      'receiver': Provider.of<DoctorProvider>(context, listen: false).doctorId,
      'senderUid': email,
      'sender': prefs.getString(kUserUid),
  ...
  void defaultInitialization()async{
    final prefs = await SharedPreferences.getInstance();
    userIdentifier =  prefs.getString(kUserUid)?? "";
    name = prefs.getString(kFirstNameConstant)!;
    token = prefs.getString(kToken)!;
    email = prefs.getString(kEmailConstant)!;
    documentId = Provider.of<DoctorProvider>(context, listen: false).patientsDocumentId;
    ...
    return GestureDetector(
      ...
      child: Scaffold(
          ...
          appBar: AppBar(
            ...
            title: Text("${Provider.of<DoctorProvider>(context, listen: false).doctorName} ", style: kNormalTextStyle.copyWith(color: kPureWhiteColor),),
            centerTitle: true,
                ...
                Padding(
                  ...
                  StreamBuilder<QuerySnapshot> (
                      stream: FirebaseFirestore.instance
                          ...
                          .where('documentId', isEqualTo: Provider.of<DoctorProvider>(context, listen: false).patientsDocumentId)
                      ...
                      builder: (context, snapshot)
                      {
                        if(!snapshot.hasData){
                          ...
                          return  messageList.length == 0 ? Padding(padding: const EdgeInsets.all(20),
                            child:
                            Center(child: Text(noMessage.tr)),) :
                          Padding(
